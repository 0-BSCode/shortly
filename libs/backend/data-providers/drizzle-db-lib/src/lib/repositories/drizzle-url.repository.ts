...
export class DrizzleUrlRepository implements IUrlRepository {
    ...
    async findById(id: number): Promise<UrlDto> {
        const record = await this.dbService.select().from(this.table).where(eq(this.table.id, id)).execute()
        
        return this.convertToDto(record[0])
    }

    async create(createDto: CreateUrlDto): Promise<UrlDto> {
        const record = await this.dbService.insert(this.table).values({
    ...
    async update(id: number, updateDto: UpdateUrlDto): Promise<UrlDto> {
        ...
        const record = await this.dbService.update(this.table).set({
            originalUrl: updateDto.originalUrl,
            clicks: updateDto.clicks
        }).where(
            eq(this.table.id, id)
        ).returning()
    ...
    async deleteById(id: number): Promise<void> {
        await this.dbService.delete(this.table).where(
            eq(this.table.id, id)
        ).execute()
    }
